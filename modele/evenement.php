...
if(empty($affiche_eventf) || ($affiche_eventf > $date)){
    $message = "Aucun évènement prévu pour le moment.";
}

$requetepasse = $dbh->prepare("SELECT ladate, titre, description, lieu, DAY(ladate) AS jour, MONTH(ladate) AS mois, YEAR(ladate) AS annee, HOUR(ladate) AS heure, MINUTE(ladate) AS minute, SECOND(ladate) AS seconde FROM evenement ORDER BY ladate DESC LIMIT 5");
